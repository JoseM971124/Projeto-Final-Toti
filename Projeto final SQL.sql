...
/*Nesta consulta, é mostrado o nome dos pacientes, 
quais processos estão sendo realizados e por que o médico e
a enfermeira foram atendidos, as tabelas Undergoes, Patient, Procedures, Physician e Nurse foram unidas*/

select patient.name as paciente, procedures.name as proceso, Physician.name AS Medico, nurse.name AS enfermera, Undergoes.DateUndergoes from undergoes
join patient
...
/*Nesta consulta, são mostrados os medicamentos fornecidos aos pacientes,
as tabelas juntas foram Prescribes, Physician, Patient, Medication, Appointmen e Nurse*/

SELECT Physician.name AS Doctor, Patient.Name AS Paciente, Medication.Name AS Medicaçao,
Prescribes.Dose AS Dose, Prescribes.Date, Nurse.Name AS Enfermeira
 FROM Prescribes
JOIN physician 
...
JOIN Appointment
ON Prescribes.Appointment = Appointment.AppointmentID
JOIN Nurse
ON Appointment.PrepNurse = Nurse.EmployeeID;

/*Quartos que estão sendo usados, indicando o número do quarto e o andar*/

select patient.name AS Paciente, stay.room AS Quarto, room.blockfloor AS Andar, stay.staystart AS "Data de entrada", stay.stayend AS 'Data de saída' from stay
JOIN patient
ON stay.patient = patient.ssn
JOIN room
ON stay.room = room.RoomNumber

/*Certificado de cada medico*/

SELECT Physician.name AS Medico, Procedures.name AS Procedimento, trained_in.CertificationDate AS "Data de certificacao", 
Trained_In.CertificationExpires AS "Data de expiração"
FROM Trained_in
JOIN Physician
ON Trained_In.Physician = Physician.EmployeeID
JOIN Procedures
ON Trained_In.Treatment = Procedures.Code;

SELECT * FROM Trained_in

select patient.ssn, patient.name, patient.address, patient.phone, patient.insuranceID, physician.name AS PCP FROM patient
JOIN Physician
ON patient.pcp = Physician.EmployeeID;

/*Consulta dos valores dos procedimentos realizados*/

select patient.name as paciente, procedures.name as proceso,procedures.cost AS Valores, Physician.name AS Medico from undergoes
join patient
on patient.ssn = undergoes.patient
join procedures 
on undergoes.procedures = procedures.code
